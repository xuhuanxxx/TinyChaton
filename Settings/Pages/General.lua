...
CategoryBuilders.general = function(rootCat)
    ...
    addon.AddProxyCheckbox(cat, P .. "enabled", L["LABEL_ENABLED"] .. L["LABEL_GLOBAL_SUFFIX"], globalDef,
    ...
    addon.AddProxyCheckbox(cat, S .. "enabled", L["LABEL_ENABLED"], shelfDef.enabled,
        function()
            local db = addon.GetTableFromPath(shelfPath)
    ...
    addon.AddProxyDropdown(cat, S .. "anchor", L["LABEL_SHELF_POSITION"], shelfDef.anchor,
        function()
    ...
    addon.AddProxyDropdown(cat, S .. "direction", L["LABEL_SHELF_DIRECTION"], shelfDef.direction,
        function()
            local c = Settings.CreateControlTextContainer()
    ...
    addon.AddProxyDropdown(cat, S .. "dynamicMode", L["LABEL_SHELF_DYNAMIC_MODE"], shelfDef.dynamicMode,
        function()
            local c = Settings.CreateControlTextContainer()
        ...
        function()
            local db = addon.GetTableFromPath(shelfPath)
            return db and db.dynamicMode or shelfDef.dynamicMode
        end,
        function(v)
            local db = addon.GetTableFromPath(shelfPath)
    ...
    addon.AddProxyDropdown(cat, S .. "theme", L["LABEL_SHELF_THEME"], shelfDef.theme,
        function()
            local c = Settings.CreateControlTextContainer()
        ...
        function(v)
            ...
            if addon.ApplyShelfSettings then addon:ApplyShelfSettings() end
            -- Refresh proxy settings when theme changes
