...
addon.ChannelAbbreviation = {}
local CA = addon.ChannelAbbreviation
...
function CA:ApplyShortChannelGlobals()
...
function CA:Init()
    ...
    if addon.TRANSFORMER_ORDER then
        ...
        for _, v in ipairs(addon.TRANSFORMER_ORDER) do
            if v == "channel_abbrev" then found = true; break end
        end
        if not found then
            table.insert(addon.TRANSFORMER_ORDER, 1, "channel_abbrev")
        end
    end
end
